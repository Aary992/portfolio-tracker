...
const HOLDINGS = [
  ...
  { name: 'PINELABS',   symbol: 'PINELABS.NS',    qty: 17,  avg: 161.94, invested: 2753.05, sector: 'Fintech' },
  { name: 'HINDCOPPER', symbol: 'HINDCOPPER.NS',  qty: 11,  avg: 480.03, invested: 5280.35, sector: 'Metals' },
  { name: 'KPITTECH',   symbol: 'KPITTECH.NS',    qty: 5,   avg: 712.85, invested: 3564.25, sector: 'Tech' },
  { name: 'E2E-BE',     symbol: 'E2E.NS',         qty: 1,   avg: 2132.00,invested: 2132.00, sector: 'Tech' },
  { name: 'ETERNAL',    symbol: 'ETERNAL.NS',     qty: 4,   avg: 275.00, invested: 1100.00, sector: 'Consumer' },
  { name: 'GROWW',      symbol: 'GROWW.NS',       qty: 12,  avg: 185.77, invested: 2229.24, sector: 'Fintech' },
      ...
      <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: C.font, fontSize: 13 }}>
        ...
        <div style={{ borderBottom: `1px solid ${C.border}`, padding: '16px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ color: C.text2, fontSize: 11 }}>NSE · Auto-refresh 20s</div>
          </div>
        </div>
          ...
          <Card style={{ padding: '20px 24px' }}>
            ...
            {P && (
              <>
                ...
                <div style={{ color: C.muted, fontSize: 9, letterSpacing: 2, textTransform: 'uppercase', margin: '14px 0 8px' }}>Simple Stats</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                  <MetricBox label="Winning Days"     value={`${(P.dailyWinRate * 100).toFixed(0)}%`}          color={P.dailyWinRate >= 0.5 ? C.green : C.amber} sub="days portfolio closed green (1y)" />
                  <MetricBox label="Best Day"         value={fmtPct(P.bestDay * 100)}                          color={C.green} sub="largest 1-day gain" />
                  <MetricBox label="Worst Day"        value={fmtPct(P.worstDay * 100)}                         color={C.red}   sub="largest 1-day loss" />
                  <MetricBox label="Longest Winning Streak" value={`${P.greenStreak} days`}                    color={C.cyan}  sub="consecutive green days" />
                </div>
              </>
            )}
          </Card>
                ...
                <LineChart data={benchmarkChartData} margin={{ left: 10, right: 20, top: 5, bottom: 5 }}>
                  ...
                  {benchmarkKeys.map(key => (
                    <Line
                      ...
                      strokeWidth={key === 'portfolio' ? 2.4 : 1.4}
                      dot={false}
                      connectNulls
        ...
        {risk?.drawdownSeries?.length > 0 && (
          <div style={{ padding: '12px 24px 0' }}>
            <Card style={{ padding: '20px 24px' }}>
              <SectionLabel sub="% below all-time high · peak-to-trough pain curve · deeper = worse">
                Drawdown (Underwater) Curve
              </SectionLabel>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={risk.drawdownSeries} margin={{ left: 10, right: 20, top: 5, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke={C.border} />
                  <XAxis dataKey="date" tick={{ fill: C.muted, fontSize: 9, fontFamily: C.font }} interval={Math.floor(risk.drawdownSeries.length / 10)} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fill: C.muted, fontSize: 9, fontFamily: C.font }} tickFormatter={v => `${v}%`} axisLine={false} tickLine={false} />
                  <Tooltip
                    contentStyle={{ background: C.card, border: `1px solid ${C.border2}`, borderRadius: 4, fontFamily: C.font, fontSize: 11 }}
                    labelStyle={{ color: C.muted }}
                    formatter={v => [`${v}%`, 'Drawdown']}
                  />
                  <Line type="monotone" dataKey="dd" stroke={C.red} fill={C.red} strokeWidth={1.5} dot={false} isAnimationActive={false} />
                </LineChart>
              </ResponsiveContainer>
            </Card>
          </div>
        ...
        {/* Risk Contribution */}
        {risk?.riskContrib?.length > 0 && (
          <div style={{ padding: '12px 24px 0' }}>
            <Card style={{ padding: '20px 24px' }}>
              <SectionLabel sub="Each holding's share of total portfolio volatility · weight × stock-vol / portfolio-vol">
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: 'left', padding: '8px 10px', color: C.muted, fontSize: 10, letterSpacing: 1, borderBottom: `1px solid ${C.border}` }}>STOCK</th>
                    <th style={{ textAlign: 'right', padding: '8px 10px', color: C.muted, fontSize: 10, letterSpacing: 1, borderBottom: `1px solid ${C.border}` }}>WEIGHT</th>
                    <th style={{ textAlign: 'right', padding: '8px 10px', color: C.muted, fontSize: 10, letterSpacing: 1, borderBottom: `1px solid ${C.border}` }}>STOCK VOL</th>
                    <th style={{ textAlign: 'right', padding: '8px 10px', color: C.muted, fontSize: 10, letterSpacing: 1, borderBottom: `1px solid ${C.border}` }}>% OF PORT VOL</th>
                    <th style={{ padding: '8px 10px', color: C.muted, fontSize: 10, letterSpacing: 1, borderBottom: `1px solid ${C.border}`, width: '40%' }}>CONTRIBUTION</th>
                  </tr>
                </thead>
                <tbody>
                  {risk.riskContrib.map(r => (
                    <tr key={r.symbol}>
                      <td style={{ padding: '8px 10px', color: C.text, borderBottom: `1px solid ${C.border}` }}>{r.name} <span style={{ color: C.muted, fontSize: 10 }}>· {r.sector}</span></td>
                      <td style={{ padding: '8px 10px', color: C.text2, textAlign: 'right', borderBottom: `1px solid ${C.border}` }}>{r.weight}%</td>
                      <td style={{ padding: '8px 10px', color: C.amber, textAlign: 'right', borderBottom: `1px solid ${C.border}` }}>{r.vol}%</td>
                      <td style={{ padding: '8px 10px', color: r.contribution > r.weight ? C.red : C.green, textAlign: 'right', borderBottom: `1px solid ${C.border}`, fontWeight: 500 }}>{r.contribution}%</td>
                      <td style={{ padding: '8px 10px', borderBottom: `1px solid ${C.border}` }}>
                        <div style={{ background: C.border, height: 6, borderRadius: 3, overflow: 'hidden' }}>
                          <div style={{ width: `${Math.min(100, r.contribution * 3)}%`, height: '100%', background: r.contribution > r.weight ? C.red : C.cyan }} />
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              <div style={{ color: C.muted, fontSize: 10, marginTop: 10 }}>
                Red = this stock contributes more risk than its weight (drag on diversification). Green = contributes less than its weight (diversifier).
              </div>
            </Card>
          </div>
        )}
